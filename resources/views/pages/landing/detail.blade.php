        ...
        <div class="grid gap-5 md:grid-cols-12">
            <main class="p-4 lg:col-span-8 md:col-span-12">
                ...
                <div class="details-heading">
                    <h1 class="text-2xl font-semibold">I Will {{ $service->title ?? '' }}</h1>
                    <div class="my-3">
                        @include('components.landing.rating')
                    </div>
                </div>
                <div class="p-3 my-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                    <img :src="featured" alt="gambar" class="rounded-lg cursor-pointer w-100" data-lity>
                        ...
                        <div class="flex mt-2 flex-nowrap">
                            ...
                            @forelse ($thumbnail as $item)
                                <img :class="{ 'border-4 border-serv-button': active === {{ $item->id }} }"
                                    ...
                                    :src="`{{ url(Storage::url($item->thumbnail)) }}`" alt="thumbnail"
                        ...
                        <div x-show.transition.duration.500ms="tab === 'seller'" class="leading-8 text-md">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        @if ($service->user->detail_user->photo != null)
                                            <img class="w-20 h-20 rounded-full"
                                                src={{ url(Storage::url($service->user->detail_user()->first()->photo)) }}
                                                alt="profil photo">
                                        @else
                                            <svg class="w-20 h-20 rounded-full text-gray-300" fill="currentColor"
                                                viewBox="0 0 24 24">
                                                <path
                            ...
                            <ul class="mb-8 list-check">
                                @forelse ($advantage_user as $advantage_user_item)
                                    <li class="pl-10 my-2">{{ $advantage_user_item->advantage ?? '' }}
                                    </li>
                                @empty
                                    {{-- empty --}}
                                @endforelse
                ...
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-center px-2 py-3 mx-4 mt-4 border rounded-full border-serv-testimonial-border">
                        ...
                        <div class="flex-1 text-sm font-medium text-center">
                            ...
                            {{ $service->service_limit ?? '' }} Revision Limit
                        </div>
                    </div>
